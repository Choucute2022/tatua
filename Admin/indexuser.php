...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_user_id'])) {
    try {
        ...
        header("Location: indexuser.php?delete_success=1&user_id=$user_id");
        exit;
    } catch (PDOException $e) {
        $pdo->rollBack(); // Hủy giao dịch nếu có lỗi
...
// Xử lý lọc dữ liệu
$sql = "SELECT id, name, email, phone_number, created_at 
        FROM users 
        WHERE 1";
$params = [];
    ...
    <main>
        ...
        <?php if (isset($_GET['delete_success'])): ?>
            <div style="color: green; margin-bottom: 16px;">
                Đã xóa khách hàng ID <?= htmlspecialchars($_GET['user_id']) ?>. 
                Các đơn hàng liên quan vẫn được giữ lại.
            </div>
        <?php endif; ?>

        <form method="GET" class="filter-form">
            <input type="text" name="search_id" placeholder="ID khách hàng" value="<?= htmlspecialchars($_GET['search_id'] ?? '') ?>">
            <input type="text" name="search_name" placeholder="Tên khách hàng" value="<?= htmlspecialchars($_GET['search_name'] ?? '') ?>">
            <input type="text" name="search_email" placeholder="Email khách hàng" value="<?= htmlspecialchars($_GET['search_email'] ?? '') ?>">
            <button type="submit">🔍 Lọc</button>
            <a href="indexuser.php" class="reset-btn" style="text-decoration: none;"><button type="button" class="reset-btn">❌ Xóa lọc</button></a>
        </form>
            ...
            <table>
                ...
                <tbody>
                    <?php if (count($users) === 0): ?>
                        <tr><td colspan="6">Không có khách hàng phù hợp.</td></tr>
                    <?php else: ?>
                        <?php foreach ($users as $user): ?>
                        <tr>
                            <td><?= htmlspecialchars($user['id']) ?></td>
                            <td><?= htmlspecialchars($user['name'] ?? 'Khách vãng lai') ?></td>
                            <td><?= htmlspecialchars($user['email']) ?></td>
                            <td><?= htmlspecialchars($user['phone_number']) ?></td>
                            <td><?= htmlspecialchars($user['created_at']) ?></td>
                            <td>
                                <form method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa khách hàng này? Các đơn hàng liên quan sẽ được giữ lại.');">
