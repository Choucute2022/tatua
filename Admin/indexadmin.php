...
$statResult['tong_so_khach_hang'] = (int)$stmt->fetchColumn();
...
<head>
    ...
    <style>
        /* Styles như bạn đã có ở trên — giữ nguyên */
        :root {
            --orange: #f39c12;
            --light-orange: #fcebd3;
            --dark-orange: #c87f0a;
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        header { background: var(--white); padding: 20px 32px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
        header h1 { color: var(--orange); font-size: 22px; }
        .container { display: flex; min-height: 100vh; }
        aside { width: 240px; background: var(--light-orange); padding: 24px; border-right: 1px solid #ffd89f; }
        aside h2 { font-size: 16px; margin-bottom: 20px; color: var(--dark-orange); }
        aside a { display: block; padding: 12px 18px; margin-bottom: 10px; border-radius: 10px; color: var(--text); text-decoration: none; font-weight: 500; transition: 0.2s; }
        aside a:hover, aside a.active { background: var(--orange); color: var(--white); font-weight: bold; }
        main { flex: 1; padding: 32px; }
        .title { font-size: 24px; color: var(--dark-orange); margin-bottom: 24px; }
        .filter select { padding: 10px 18px; border-radius: 10px; border: 1px solid #f5d3a8; background: var(--white); font-size: 15px; margin-bottom: 24px; }
        .stats { display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 32px; }
        .card { background: linear-gradient(135deg, #f39c12, #f5b041); color: var(--white); flex: 1; min-width: 220px; padding: 20px; border-radius: 14px; box-shadow: 0 6px 14px rgba(0,0,0,0.1); }
        .card h3 { font-size: 14px; opacity: 0.9; text-transform: uppercase; margin-bottom: 6px; }
        .card p { font-size: 26px; font-weight: bold; }
        .chart-container { background: var(--white); padding: 28px; border-radius: 16px; box-shadow: 0 6px 12px rgba(0,0,0,0.05); }
        footer { text-align: center; padding: 18px; font-size: 14px; background: var(--white); border-top: 1px solid #eee; color: #a05e00; }
...
<script>
    Toastify({
        text: <?= json_encode($_SESSION['toast_message']) ?>,
        duration: 3000,
        gravity: "top",
        position: "right",
